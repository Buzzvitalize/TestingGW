...
const TOKEN = process.env.GW_DEMO_TOKEN || 'gw-phase8-demo';
...
const metrics = { opened: 0, closed: 0, rx: 0, tx: 0, matches: 0, tournaments: 0, simulations: 0 };

const now = () => Date.now();
const id = () => crypto.randomUUID().slice(0, 8);

function createPlayer() {
  return { id: `p-${id()}`, name: `Player-${Math.floor(Math.random() * 1000)}`, authenticated: false, roomId: null, x: 80, y: 80, score: 0, wins: 0, lastActionAt: 0 };
}
...
function ensureRoom(roomId) {
  ...
  const room = {
    id: roomId,
    npc: { id: 'npc-arena8', hp: 300, maxHp: 300, x: 420, y: 220 },
    players: new Set(),
    tournament: { round: 1, finished: false },
    createdAt: now(),
  };
...
function pushEvent(roomId, event) {
  ...
  arr.push({ ts: now(), ...event });
  if (arr.length > 800) arr.shift();
  eventsByRoom.set(roomId, arr);
}
...
function roomSnapshot(room) {
  return {
    ...
    players: Array.from(room.players)
      ...
      .filter(Boolean)
      .map((c) => ({ id: c.player.id, name: c.player.name, x: c.player.x, y: c.player.y, score: c.player.score, wins: c.player.wins })),
  };
}
...
function buildMatch() {
  while (queue.length >= 2) {
    ...
    metrics.matches += 1;
    pushEvent(roomId, { type: 'match_created', roomId, players: [clients.get(a).player.id, clients.get(b).player.id] });
    broadcastRoom(roomId, { type: 'match_found', room: roomSnapshot(room) });
  }
}

function evaluateTournament(room, scorer) {
  if (scorer.score < 120) return;
  room.tournament.round += 1;
  scorer.wins += 1;
  scorer.score = 0;
  room.npc.hp = room.npc.maxHp;
  pushEvent(room.id, { type: 'tournament_round_win', playerId: scorer.id, round: room.tournament.round, wins: scorer.wins });

  if (room.tournament.round > 3) {
    room.tournament.finished = true;
    metrics.tournaments += 1;
    pushEvent(room.id, { type: 'tournament_finished', playerId: scorer.id, wins: scorer.wins });
  }
}
...
function applyAction(client, payload) {
  ...
  if (payload.type === 'move') {
    ...
    pushEvent(room.id, { type: 'move', playerId: p.id, x: p.x, y: p.y });
    return { type: 'ack', action: 'move', x: p.x, y: p.y };
  }

  if (payload.type === 'attack') {
    room.npc.hp = Math.max(0, room.npc.hp - 20);
    p.score += 20;
    pushEvent(room.id, { type: 'attack', playerId: p.id, dealt: 20, npcHp: room.npc.hp, score: p.score });

    if (room.npc.hp === 0) {
      room.npc.hp = room.npc.maxHp;
      p.score += 40;
      pushEvent(room.id, { type: 'npc_respawn', by: p.id, score: p.score });
    }

    evaluateTournament(room, p);
    return { type: 'combat', npcHp: room.npc.hp, score: p.score, wins: p.wins, round: room.tournament.round };
  }

  return { type: 'error', error: 'unsupported_action' };
}

function simulateRoom(roomId, ticks = 12) {
  const room = ensureRoom(roomId);
  for (let i = 0; i < ticks; i += 1) {
    pushEvent(room.id, { type: 'sim_tick', n: i + 1, npcHp: room.npc.hp });
    room.npc.hp = Math.max(0, room.npc.hp - 5);
    if (room.npc.hp === 0) room.npc.hp = room.npc.maxHp;
  }
  metrics.simulations += 1;
  return { roomId: room.id, ticks, npcHp: room.npc.hp };
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/rooms') {
    const roomsList = Array.from(rooms.values()).map((r) => ({ id: r.id, players: r.players.size, npcHp: r.npc.hp, round: r.tournament.round, finished: r.tournament.finished }));
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ts: now(), rooms: roomsList }));
    return;
  }
  ...
  if (req.url.startsWith('/simulate?')) {
    ...
    const roomId = q.get('roomId') || 'sim-room';
    const ticks = Number(q.get('ticks') || 12);
    const result = simulateRoom(roomId, ticks);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, result }));
    return;
  }

  if (req.url.startsWith('/events.ndjson?roomId=')) {
    const roomId = decodeURIComponent(req.url.split('=')[1] || '');
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(JSON.stringify({ event: 'phase8_gateway_started', port: PORT, tokenHint: TOKEN }));
});
